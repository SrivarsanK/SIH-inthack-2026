...
export const LiveMap: React.FC<LiveMapProps> = ({ data, selectedAgency }) => {
  ...
  const initialCenter = [stops[0].lat, stops[0].lon];
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      // Glow line
      const outerLine = L.polyline(polylineCoords, {
        color: selectedAgency?.accentColor || "#3b82f6",
        weight: 8,
        opacity: 0.3,
        lineCap: "round"
      }).addTo(map);
      polylineGlowRef.current = outerLine;
          ...
          <div class="group relative flex items-center justify-center">
            ...
            <div class="relative flex items-center justify-center w-7 h-7 rounded-full ${
              isTerminal 
                ...
                : 'bg-slate-900 text-white border border-slate-700 text-[11px] font-bold shadow-md'
            }">
              ${i + 1}
            </div>
          </div>
  ...
  const handleZoom = (type: "in" | "out" | "reset") => {
    ...
    if (type === "reset") mapInstanceRef.current.setView(initialCenter as [number, number], 13);
  };
